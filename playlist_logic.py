...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    is_chill_keyword = any(k in genre for k in chill_keywords)  # Fixed: check genre not title
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    total = len(all_songs)  # Fixed: use total of all songs, not just hype
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        total_energy = sum(song.get("energy", 0) for song in all_songs)  # Fixed: sum all songs, not just hype
...
def search_songs(
    ...
    for song in songs:
        ...
        if value and q in value:  # Fixed: inverted logic - query should be IN value
...
def random_choice_or_none(songs: List[Song]) -> Optional[Song]:
    ...
    if not songs:  # Fixed: handle empty list gracefully
        return None
